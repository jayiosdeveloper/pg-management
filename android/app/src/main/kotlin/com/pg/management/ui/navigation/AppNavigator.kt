...
import com.pg.management.ui.screens.admin.workers.WorkerCredentialsScreen
import com.pg.management.ui.screens.admin.workers.WorkerFormScreen
...
fun AppNavigator(startDestination: String) {
    ...
    NavHost(
        ...
        composable(Routes.LOGIN) {
            LoginScreen(
                onLoggedInAsAdmin = { nav.navigate(Routes.ADMIN_HOME) { popUpTo(Routes.LOGIN) { inclusive = true } } },
                onLoggedInAsTenant = { nav.navigate(Routes.TENANT_HOME) { popUpTo(Routes.LOGIN) { inclusive = true } } },
        ...
        composable(Routes.ADMIN_HOME) {
            AdminShellScreen(
                onLogout = { nav.navigate(Routes.LOGIN) { popUpTo(Routes.ADMIN_HOME) { inclusive = true } } },
        ...
        composable(
            ...
            WorkerFormScreen(onBack = { nav.popBackStack() }, onSaved = { nav.popBackStack() })
        }

        composable(
            route = Routes.WORKER_CREDENTIALS,
        ...
        composable(Routes.TENANT_HOME) {
            TenantHomeScreen(
                onLogout = { nav.navigate(Routes.LOGIN) { popUpTo(Routes.TENANT_HOME) { inclusive = true } } },
            )
        }
    }
}
